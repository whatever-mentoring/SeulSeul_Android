...
class EndLocationRepo @Inject constructor(
    private val apiService: ApiService
) {
    // location: Location 객체를 받아서, 서버에 Request하고, Result<T> 객체를 Response한다(LocationData).
    suspend fun postEndLocation(location: EndLocationRequest): Result<EndLocationData> {
        return try {
            // 서버에 post request
            val response = apiService.postEndLocation(location)
            // response 성공 여부 확인
            if (response.isSuccessful) {
                // 성공 로그 -> response.body()데이터 전체 추출
                Log.d("smj", "Post end location successful ${response.body()}")
            ...
            // 예외 로그
            Log.e("smj", "Exception when posting location", e)
            Result.failure(e)
        }
    }
}
