package com.timi.seulseul.presentation.main.activity
...
import com.timi.seulseul.presentation.main.viewmodel.MainViewModel
...
class MainActivity : BaseActivity<ActivityMainBinding>(R.layout.activity_main) {
    ...
    override fun onResume() {
        ...
        // 거주지 닉네임 설정
        val endNickName = BaseActivity.prefs.getString(END_NICKNAME, "")
    ...
    private fun checkSwitchSetting() {
        ...
        if (prefs.getInt(ALARM_TIME, 0) != 0 || !prefs.getBoolean(REFRESH, true)) {
            showSubwayRoute()
        }
        ...
        if (alarmOn) {
            ...
            setAlarmAfterTitle()
            prefs.edit().putBoolean(TODAY_ALARM, true).apply()
            ...
            if (prefs.getInt(ALARM_TIME, 0) != 0 && !prefs.getBoolean(TODAY_ALARM, false)) {
                Timber.d("경로 show시간 되서 알림 off : ${prefs.getBoolean(TODAY_ALARM, false)}")
                setAlarmBeforeTitle()
            }
        }
    }
